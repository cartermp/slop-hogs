...
At database growth thresholds, close registrations and card rendering before entering read-only mode. Do not delete hog histories to recover space. The detailed thresholds are in the implementation plan; database measurement and enforcement belong with persistence.

## If usage spikes

Close signup and disable external work through implemented controls. Inspect Railway usage and logs without printing secrets. Stop the app if necessary. Inspect database and backup storage separately. Never add restart loops or raise limits to get around a cutoff. The owner decides when and how to resume.
