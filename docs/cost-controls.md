...
The app has Bluesky OAuth login plus a cheap `/api/health` route. Login initiation is limited to five attempts per trusted client address per hour and 200 globally per hour using atomic PostgreSQL counters. Handle suggestions are limited to 60 searches per trusted client address per hour and 1,000 globally per hour, with five results per request. Any account verified by Bluesky OAuth can sign in and receive a hog. Before database authentication, combined farm traffic is limited to 1,200 requests per trusted client address per minute with a burst of 40 and 2,400 globally per minute with a burst of 80. Source addresses are hashed, and bounded source-bucket storage evicts the oldest entry rather than rejecting every new client. After authentication, farm syncs are limited to 90 requests per account per minute with a burst of five; game actions allow 600 per account per minute with a burst of 20 so ordinary movement remains responsive. These defense-in-depth limits are appropriate because production is fixed to one replica. Farm action bodies stop at 1 KB, all Server Action bodies stop at 64 KB, visible farm polling is serialized, and hidden tabs neither poll nor submit movement. Authenticated public-post previews reserve at most 12 new lookups per account per UTC day and 100 globally per hour before contacting Bluesky. Fresh seven-day cached previews do not spend quota. Visitor treats perform no external work and are limited to three sent per account and ten received per pen each UTC day, one sender-to-pen gift per day, and 20 pending gifts per active hog. Authenticated share-card requests reserve at most two attempts per account and 50 globally per UTC day, serialize local Sharp work to one card per app instance, stop each PNG at 250 KB, and stop stored card data at 250 MB. Failed renders spend their reservation; successful retries read the immutable card without spending another. OAuth discovery, actor search, post lookup, and card rendering each have a five-second deadline; card rendering uses only authored text and local SVG assets. Imports and model calls remain disabled.

The internal database budget is 1 GB. Startup records an actual PostgreSQL database-size measurement. Game writes refresh it at most once every 15 minutes; `/owner` refreshes it on demand. At 70% the owner page warns, at 85% future card creation stops, and at 95% new game state changes stop. `features.readOnlyMode` immediately blocks future card creation and new game state changes after deploying a reviewed policy change. Measurements and effective controls persist across restarts.
