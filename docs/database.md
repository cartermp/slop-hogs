...
The integration test uses actual row locks and concurrent requests. Eight provisioning requests must create one active hog. Eight identical feeds must consume one meal and return identical saved events. Twelve further feeds must accept exactly five. A forced transaction failure must preserve saved state. A new connection pool must read identical state and return the original retry receipt. Expired, revoked, and other-owner sessions must fail. OAuth admission must reject closed or storage-blocked registration, preserve a returning account's hog, and rotate its application session. Read-only mode rejects new state changes while preserving an existing idempotent receipt. Restore verification must reject the source database as its own restore target.
