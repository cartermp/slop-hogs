...
SH-005 exposes login and callback routes but keeps provisioning and session issuance behind the verified OAuth callback. The callback uses the SDK-returned DID, never a browser-supplied DID, and atomically creates or reuses the account, ensures one active hog, and rotates the application session.
...
The integration tests use actual row locks and concurrent requests. Eight provisioning requests must create one active hog. Eight identical feeds must consume one meal and return identical saved events. Twelve further feeds must accept exactly five. A forced transaction failure must preserve saved state. A new connection pool must read identical state and return the original retry receipt. Expired, revoked, and other-owner sessions must fail. OAuth sign-in must provision a verified DID, preserve a returning account's hog, and rotate its application session. Mutation discovery and cleaning share the same atomic, idempotent receipt path. Opening a hog displays elapsed time without writing or rolling discoveries. Concurrent terminal retries must create one ending row and one frozen tombstone; concurrent replay requests must create one next generation while retaining the prior life under the same public pen. Account sessions remain valid between generations even though active-hog actions do not. Public-pen tests prove that queued gifts do not change state, recipient and sender limits remain atomic, only the owner can accept a gift, and blocks and visibility controls stop interaction. Farm tests prove multiple players share a snapshot, concurrent pickup claims have exactly one winner, popping persists, and restarting resets the same account's farm player. Read-only mode rejects legacy state changes while preserving an existing idempotent receipt or generation. Restore verification must reject the source database as its own restore target.
Farm achievement tests also verify that lifetime counters survive a restart,
...
Migrations run explicitly, never on web startup or build. Numbered SQL files are applied in one transaction under an advisory lock, with checksums that reject edits to applied files. Add a new migration for changes. Migration 005 upgrades every active and ended rules-version-1 hog state with the version-2 mutation, recent-diet, and cleaning fields; historical action receipts remain immutable. Migration 006 adds opaque public pen IDs, per-account pen controls, gift and block records, and daily sender and recipient gift counters. Migration 007 adds durable share events and speech drafts, immutable PNG storage, render state, and daily account and global card counters. Migration 008 upgrades rules-version-2 states, numbers existing lives by generation, and adds terminal snapshots bound to their triggering action receipts. Migration 009 adds one persistent multiplayer farm avatar per account and a bounded set of expiring slop drops. Migrations 010 through 012 add decaying psychosis, player identity, farm achievements, and movement-only psychosis decay; migration 013 adds battle health, knockouts, attack cooldowns, and defeated states, migration 014 adds persistent knockout achievement progress, migration 015 adds multiple authentication providers, and migration 016 groups active players and slop into bounded matchmade fields. Before deployment, back up the database and test each migration against a copy.
