...
`config/cost-policy.json` holds the approved initial limits. Startup refuses missing, malformed, or unsafe settings. Any Bluesky account verified by OAuth can create a Slop Hogs account. Per-account token buckets bound farm sync and action traffic while preserving the normal real-time movement cadence; polling pauses in hidden tabs and never stacks concurrent sync requests. Farm action bodies stop at 1 KB and Server Action bodies at 64 KB. New public lookups are limited to 12 per account per UTC day and 100 globally per hour; cached previews do not spend lookup quota. Visitor treats are limited to three sent per account per UTC day, ten received per pen per day, one sender-to-pen gift per day, and 20 pending gifts per active hog. Authenticated owners can render at most two new cards per UTC day and the app can render 50 globally; failed render attempts count, one render runs per app instance, PNGs stop at 250 KB each, and stored cards stop at 250 MB total. The app records database size at startup and at most every 15 minutes during writes. It warns at 70% of the 1 GB internal budget, blocks new cards at 85%, and rejects new game state changes at 95%. `features.readOnlyMode` is the manual emergency stop.
