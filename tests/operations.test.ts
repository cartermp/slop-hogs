...
test("database growth thresholds warn, restrict creation, then make writes read-only", () => {
  assert.deepEqual(
  ...
  assert.equal(restricted.registrationsBlocked, true);
...
test("manual read-only mode overrides storage and invalid measurements fail closed", () => {
  ...
  assert.equal(manual.registrationsBlocked, true);
